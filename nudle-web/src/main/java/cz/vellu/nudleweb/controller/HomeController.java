...
public class HomeController {
    ...
    @PostMapping("/order")
    public String submitForm(@ModelAttribute @Valid EmailContentModel emailModel, Model model, BindingResult result) {

        if (result.hasErrors()) {
            model.addAttribute("failure", "Prosim zkontrolujte si zadane udaje.");
            throw new ValidationException("Validation errors when submitting form: " + result.getErrorCount());
        } else {
            try {
